...
| `/rrhh/staff/[id]` | `canAccessHrModule` (rrhh only) | Ficha del colaborador (US-67): header (breadcrumb + Avatar + estado + chip de vínculo + CTAs `+ Nuevo contrato`, `Editar datos`), dos columnas — izquierda con "Datos personales" (dl: nombre, DNI, CUIT, vínculo, fecha alta), "Contacto" (email + teléfono), "Datos bancarios" (CBU/alias), "Contratos" (lista con link a `/rrhh/contracts/[id]`) y "Liquidaciones" — derecha con "Antigüedad en el club" (card accent-rrhh con años · meses + fecha alta) y totales año/mes. Modal "Editar colaborador" invocable via CTA `Editar datos` usa `updateStaffMemberAction`. |
