#!/usr/bin/env python3
"""Fase 1 — Parse y valida CSV 2024 contra DB.

Lee el CSV `Movimientos-Grid View (1).csv`, normaliza los datos y clasifica
...
Uso:
    python3 scripts/import-2024/parse_csv.py [--csv PATH]

Output:
    out/2024_summary.json
    out/2024_normalized.json
    out/2024_classification_report.md
"""
...
DEFAULT_CSV = "/Users/joaquin/Downloads/Movimientos-Grid View (1).csv"
...
def parse_cost_centers(raw):
    """'CC-330: ...,CC-336: ...' → [('CC-330', 'Presupuesto 1ra AFA 2024'), ...]."""
    if not raw:
        return []
    out = []
...
def main():
    ...
    summary = {
        ...
        "contracts_no_coverage_2024": Counter(),
                        ...
                        for m in member_candidates:
                            for c in contracts_by_member.get(m["id"], []):
                                ...
                                if start <= ref_date <= end:
                                    eligible.append(c)
                        if not eligible:
                            # Fallback: any contract that touches 2024
                            for m in member_candidates:
                                for c in contracts_by_member.get(m["id"], []):
                                    start = date.fromisoformat(c["start_date"])
                                    end = date.fromisoformat(c["end_date"]) if c.get("end_date") else date(9999, 12, 31)
                                    if start <= target_end and end >= target_start:
                                        eligible.append(c)
                        if not eligible:
                            summary["contracts_no_coverage_2024"][contract_raw] += 1
                        elif len(eligible) > 1:
    ...
    norm_path_tmp = OUT / "2024_normalized.json"
    ...
    summary_path = OUT / "2024_summary.json"
    ...
    report_path = OUT / "2024_classification_report.md"
    lines = ["# Clasificación de grupos TRX — CSV 2024", ""]
    ...
    for c, n in sorted(summary["contracts_unknown"].items(), key=lambda x: -x[1])[:20]:
        print(f"  {n:5}  {c}")
    print(f"\nContracts NO_COVERAGE_2024 ({sum(summary['contracts_no_coverage_2024'].values())} rows):")
    for c, n in sorted(summary["contracts_no_coverage_2024"].items(), key=lambda x: -x[1])[:20]:
        print(f"  {n:5}  {c}")
