...
- **Mirrors Tesorería (`/treasury/payroll`, `/treasury/staff/[id]`, `/treasury/reports/payroll`)**: vistas espejo del bounded context RRHH expuestas dentro del módulo Tesorería para rol `tesoreria` (E04 · US-45/46/48). Solo lectura excepto la bandeja `/treasury/payroll`, donde Tesorería puede pagar (US-42/43) o devolver una liquidación a "generada" (US-41). El módulo `/rrhh` sigue cerrado para Tesorería — no hay duplicación de modelo: los services subyacentes (`getStaffProfile`, `getHrReport`) usan guards "permisivos" (`canViewStaffProfile`, `canViewHrReports`) y los guards de cada página (`canAccessTreasuryStaffProfile`, `canAccessTreasuryPayrollTray`, `canAccessTreasuryPayrollReports`) deciden quién entra a qué ruta. Detalle en [`docs/pdd/pdd_us_46_47_48_vistas_paralelas_treasury.md`](docs/pdd/pdd_us_46_47_48_vistas_paralelas_treasury.md).
