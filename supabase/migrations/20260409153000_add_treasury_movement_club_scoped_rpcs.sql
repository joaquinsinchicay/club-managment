create or replace function public.get_treasury_movements_by_account_and_date_for_current_club(
  p_club_id uuid,
  p_account_id uuid,
...
create or replace function public.get_treasury_movements_by_date_for_current_club(
  p_club_id uuid,
  p_movement_date date
)
returns table (
...
create or replace function public.get_treasury_movement_by_id_for_current_club(
  p_club_id uuid,
  p_movement_id uuid
)
returns table (
...
begin
  ...
  from public.treasury_movements tm
  where tm.club_id = p_club_id
    and tm.movement_date::text like p_year || '-%';
end;
$$;
