...
| `/rrhh/contracts/[id]` | `canAccessHrMasters` (rrhh only) | Ficha del contrato: header (breadcrumb + código `C-XXXXXXXX` + Avatar + estado + chips), dos columnas — izquierda con "Información del contrato" (dl: número, tipo de pago, estructura, división, rol, actividad, tipo de remuneración, inicio, fin), "Historial de revisiones" (timeline con % vs anterior, vigente destacada), "Documentos" (upload/download/delete vía `uploadContractAttachmentAction` / `deleteContractAttachmentAction` / signed URL) — derecha con "Monto vigente · {tipoPago}" (monto grande + revisado en + delta %) y "Últimas liquidaciones" (hasta 5, vía `listSettlementsForContract`). CTAs: `createSalaryRevisionAction` (US-34), `finalizeStaffContractAction`, link a `/rrhh/staff/[staffMemberId]`. |
