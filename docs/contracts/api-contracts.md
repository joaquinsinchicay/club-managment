...
| `/rrhh/contracts/[id]` | `canAccessHrMasters` (rrhh only) | Ficha del contrato: header (breadcrumb + código `C-XXXX` + Avatar + estado + chips), dos columnas — izquierda con "Información del contrato" (dl: número, tipo, estructura, división, rol, inicio, fin), "Historial de revisiones" (timeline con rango `MesCorto Año → MesCorto Año/Vigente`, bullet pink para actual, % vs anterior, motivo, monto), "Documentos" (upload/download/delete vía `uploadContractAttachmentAction` / `deleteContractAttachmentAction` / signed URL) — derecha con "Monto vigente · {tipoPago}" (card accent-rrhh + monto grande + revisado en + delta %) y "Últimas liquidaciones" (hasta 5, vía `listSettlementsForContract`). Modal "Nueva revisión" (US-34) con subtítulo `staff · rol · actividad`, preview `MONTO VIGENTE → NUEVO MONTO`, toggle de cálculo `% variación | Monto nuevo`, fecha vigencia, motivo preset (paritaria/trimestral/inflación/promoción/corrección/otro) + observaciones textarea — el monto final computado se envía al RPC. CTAs: `createSalaryRevisionAction`, `finalizeStaffContractAction`, link a `/rrhh/staff/[staffMemberId]`. |
...
| `/rrhh/staff` | `canAccessHrMasters` (rrhh only) | `staff_members` + form alta (modal) + alerta US-60. Default filter: "con contrato vigente". Sin edición directa desde el listado — cada fila expone icon "Ver ficha" que navega a `/rrhh/staff/[id]`. |
...
| `/rrhh/staff/[id]` | `canAccessHrModule` (rrhh only) | Ficha del colaborador (US-67): header (breadcrumb + Avatar + estado + chip de vínculo + CTAs `+ Nuevo contrato`, `Editar datos`, `Dar de baja…`), dos columnas — izquierda con "Datos personales" (dl: nombre, DNI, CUIT, vínculo, fecha alta), "Contacto" (email + teléfono), "Datos bancarios" (CBU/alias), "Contratos" (lista tipo card `{C-XXXX · Rol · Estructura}` + chips de estado/tipo + monto con unidad `/mes \| /hora \| /clase`; header con `+ Nuevo` accent-rrhh sm) y "Liquidaciones" — derecha con "Antigüedad en el club" (card accent-rrhh) + totales año/mes. Modales invocables desde el header: `Nuevo contrato` (reusa `CreateContractForm` con el colaborador pre-llenado) via `createStaffContractAction`; `Editar datos` via `updateStaffMemberAction`; `Dar de baja…` (destructive-outline; deshabilitado si hay contratos vigentes) via `deactivateStaffMemberAction` → RPC `hr_deactivate_staff_member`. Cuando `deactivated_at !== null`, la ficha muestra banner warning con fecha + motivo y oculta los 3 CTAs mutadores. |
