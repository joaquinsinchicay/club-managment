...
- **Mirrors Tesorería (`/treasury/payroll`, `/treasury/staff/[id]`)**: vistas espejo del bounded context RRHH expuestas dentro del módulo Tesorería para rol `tesoreria` (E04 · US-71 / US-67). Solo lectura excepto la bandeja `/treasury/payroll`, donde Tesorería puede pagar (US-64/65) o devolver una liquidación a "generada" (US-70). El módulo `/rrhh` sigue cerrado para Tesorería — no hay duplicación de modelo: el service subyacente `getStaffProfile` usa un guard "permisivo" (`canViewStaffProfile`) y los guards de cada página (`canAccessTreasuryStaffProfile`, `canAccessTreasuryPayrollTray`) deciden quién entra a qué ruta. Detalle en `docs/pdd/pdd_us_67_ficha_colaborador.md` § 16 y `pdd_us_68_dashboard_rrhh.md` § 16. Equivalencias Notion ↔ repo en [`docs/pdd/README.md`](docs/pdd/README.md). Nota: la pantalla `/rrhh/reports` y su mirror `/treasury/reports/payroll` (US-69) fueron eliminadas el 2026-04-28; sus comparativas se cubren ahora dentro del Resumen RRHH (US-68).
