...
- **Módulo RRHH (`/rrhh`)**: todo lo del bounded context RRHH (Resumen, Contratos, Colaboradores, Estructuras, Liquidaciones, Reportes, Fichas). **Acceso restringido al rol `rrhh` exclusivo** — ni `admin` ni `tesoreria` ni `secretaria` ven el módulo ni pueden invocar sus endpoints. Todos los guards (`canAccessHrModule`, `canAccessHrMasters`, `canMutateHrMasters`, `canOperateHrSettlements`, `canOperateHrPayments`) delegan en el mismo check. **Los maestros (Estructuras, Colaboradores, Contratos) NO viven en `/settings`** — el Coordinador de RRHH los administra directamente desde el módulo.
- **Mirrors Tesorería (`/treasury/payroll`, `/treasury/staff/[id]`, `/treasury/reports/payroll`)**: vistas espejo del bounded context RRHH expuestas dentro del módulo Tesorería para rol `tesoreria` (E04 · US-71 / US-67 / US-69). Solo lectura excepto la bandeja `/treasury/payroll`, donde Tesorería puede pagar (US-64/65) o devolver una liquidación a "generada" (US-70). El módulo `/rrhh` sigue cerrado para Tesorería — no hay duplicación de modelo: los services subyacentes (`getStaffProfile`, `getHrReport`) usan guards "permisivos" (`canViewStaffProfile`, `canViewHrReports`) y los guards de cada página (`canAccessTreasuryStaffProfile`, `canAccessTreasuryPayrollTray`, `canAccessTreasuryPayrollReports`) deciden quién entra a qué ruta. Detalle en `docs/pdd/pdd_us_67_ficha_colaborador.md` § 16, `pdd_us_68_dashboard_rrhh.md` § 16, `pdd_us_69_reportes_rrhh.md` § 16. Equivalencias Notion ↔ repo en [`docs/pdd/README.md`](docs/pdd/README.md).
