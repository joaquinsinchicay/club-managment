...
mov_groups AS (
  SELECT
    ...
    (ARRAY_AGG(id ORDER BY (regexp_replace(external_id, '^IMP2024-', ''))::int))[1]
...
inserted AS (
  ...
  SELECT
    ...
    CASE WHEN mg.movs_count > 1
         THEN 'Carga historica CSV 2024 - agrupa ' || mg.movs_count || ' pagos parciales del mes.'
         ELSE 'Carga historica CSV 2024.'
    END,
    NOW(), NOW()
  FROM mov_groups mg
