#!/usr/bin/env python3
"""Fase 1 — Parse y valida CSV 2023 contra DB.
...
Uso:
    python3 scripts/import-2023/parse_csv.py [--csv PATH]

Output:
    out/2023_summary.json
    out/2023_normalized.json
    out/2023_classification_report.md
"""
...
ACCOUNT_MAP = {
    ...
    "fci": "FCI",
    "pro-tesoreria": "Pro-Tesoreria",         # NUEVO 2023 — debe crearse
    "mobbex": "Mobbex",                        # NUEVO 2023 — debe crearse
}
...
def parse_cost_centers(raw):
    """'CC-330: ...,CC-336: ...' → [('CC-330', 'Presupuesto 1ra AFA 2023'), ...]."""
    if not raw:
        return []
    out = []
...
def main():
    ...
    summary = {
        ...
        "contracts_no_coverage_2023": Counter(),
            ...
            if contract_raw:
                ...
                if not parsed:
                    ...
                    if not member_candidates:
                        # Fallback: matchear solo por last_name (caso CORDARY sin firstname en CSV)
                        ...
                        for m in member_candidates:
                            for c in contracts_by_member.get(m["id"], []):
                                ...
                                if start <= ref_date <= end:
                                    eligible.append(c)
                        if not eligible:
                            # Fallback: any contract that touches 2023
                            for m in member_candidates:
                                for c in contracts_by_member.get(m["id"], []):
                                    start = date.fromisoformat(c["start_date"])
                                    end = date.fromisoformat(c["end_date"]) if c.get("end_date") else date(9999, 12, 31)
                                    if start <= target_end and end >= target_start:
                                        eligible.append(c)
                        if not eligible:
                            summary["contracts_no_coverage_2023"][contract_raw] += 1
                        elif len(eligible) > 1:
    ...
    summary["trx_classification"] = {
        "transfer": 0, "fx_operation": 0, "anomaly_solo": 0, "anomaly_pair_imbalanced": 0
    }
    ...
    for trx_id, members in trx_groups.items():
        ...
        if n == 1:
            ...
            # No encaja en ningún bucket conocido — abortar para review manual
            print(f"\n❌ Grupo TRX {trx_id} con composición desconocida ({n} filas, currencies={currencies}, "
                  f"egresos={len(egresos)}, ingresos={len(ingresos)}). Abortando.", file=sys.stderr)
            sys.exit(2)

    # Re-write JSON normalizado con kinds resueltos
    norm_path_tmp = OUT / "2023_normalized.json"
    ...
    summary_path = OUT / "2023_summary.json"
    ...
    report_path = OUT / "2023_classification_report.md"
    lines = ["# Clasificación de grupos TRX — CSV 2023", ""]
    ...
    for c, n in sorted(summary["contracts_unknown"].items(), key=lambda x: -x[1])[:20]:
        print(f"  {n:5}  {c}")
    print(f"\nContracts NO_COVERAGE_2023 ({sum(summary['contracts_no_coverage_2023'].values())} rows):")
    for c, n in sorted(summary["contracts_no_coverage_2023"].items(), key=lambda x: -x[1])[:20]:
        print(f"  {n:5}  {c}")
