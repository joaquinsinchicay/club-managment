-- Allow `tesoreria` role to register account transfers.
-- Extends the transfer RPC with an explicit origin role and widens the RLS
-- policies on account_transfers and treasury_movements inserts.
...
create or replace function public.create_account_transfer_for_current_club(
  ...
  p_origin_role public.movement_origin_role default 'secretaria'
)
returns table (
  transfer_id uuid,
  club_id uuid,
...
-- ============================================================================
-- RLS: account_transfers INSERT — allow secretaria and tesoreria
-- ============================================================================

drop policy if exists "Secretaria can insert account transfers in current club" on public.account_transfers;
drop policy if exists "Secretaria or tesoreria can insert account transfers in current club" on public.account_transfers;
...
-- ============================================================================
-- RLS: treasury_movements INSERT — allow tesoreria for transfer-origin rows
-- ============================================================================
-- The RPC runs with security invoker, so the INSERT is performed by the
-- authenticated user. Transfers initiated by tesoreria must be accepted.

drop policy if exists "Secretaria can insert movements in current club" on public.treasury_movements;
