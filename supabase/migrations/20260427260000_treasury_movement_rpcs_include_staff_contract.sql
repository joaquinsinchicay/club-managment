-- Extender los RPCs de movimientos de tesoreria para devolver
-- staff_contract_id (US-67/US-71 + carga historica del CSV 2021).
--
-- Necesario para que el edit modal del front pueda mostrar el contrato
-- linkeado al movimiento. Sin este campo en el RETURNS TABLE, el repository
-- mapper no puede exponerlo y la UI siempre lo trata como ausente.
--
-- Backward compatible: solo AGREGA una columna al final del row shape.
-- Los callers que usen mapper por nombre (no por orden) siguen funcionando.
-- Postgres requiere DROP antes de cambiar el RETURNS TABLE de una funcion.
...
begin
  ...
  from public.treasury_movements tm
  where tm.club_id = p_club_id
    and tm.account_id = any(p_account_ids)
  order by tm.created_at desc, tm.id desc;
end;
$function$;
