#!/usr/bin/env python3
"""Fase 4 — Import movimientos 2024 via Supabase REST.

Lee out/2024_normalized.json (producido por parse_csv.py), construye los
...
Idempotente: cada mov usa external_id único 'IMP2024-<csv_id>'. Re-correr
con datos ya importados aborta el script (pre-flight check).

UUIDs determinísticos via uuid5 → mismas semillas dan mismos IDs.

Uso: python3 scripts/import-2024/rest-import.py [--dry-run] [--limit N]
"""
...
NS_MOV = uuid_lib.UUID("00000000-0000-0000-0000-000000202400")
NS_TRANSFER = uuid_lib.UUID("00000000-0000-0000-0000-000000202401")
NS_FX = uuid_lib.UUID("00000000-0000-0000-0000-000000202402")
...
def build_movement(r, club_id, override_concept=None, override_category_id="__use_subcat__",
    ...
    return {
        ...
        "status": "posted",
        "display_id": r["external_id"],  # temporal IMP2024-N (renumerado en Fase 4.5)
...
def main():
    ...
    norm_path = OUT / "2024_normalized.json"
    ...
    # Pre-flight: 0 IMP2024-* en DB
    if not args.dry_run:
        existing = get(env, "treasury_movements", {
            "external_id": "like.IMP2024-%", "select": "id", "limit": "1",
        })
        if existing:
            print("⚠️  Ya hay movimientos IMP2024-* en DB. Abortando.")
            sys.exit(2)
    ...
    transfers_multi_grouped = defaultdict(list)  # NUEVO 2024
    ...
    anomaly_invalid = [r for r in rows if r.get("kind") == "anomaly_invalid_composition"]  # NUEVO 2024
    anomaly_imbalanced = [r for r in rows if r.get("kind") == "anomaly_imbalanced"]  # NUEVO 2024
    for r in rows:
    ...
    # 2.5) Multi-source transfers (NUEVO 2024): 1 account_transfer + N movs hijos
    for trx_id, members in transfers_multi_grouped.items():
        ...
        transfer_concept = ingreso.get("concept") or f"Transferencia multi-source {trx_id}"
        transfers.append({
            "id": transfer_id,
            "club_id": club_id,
            "source_account_id": egresos[0]["account_id"],  # primer egreso (otros preservan account_id)
            "target_account_id": ingreso["account_id"],
    ...
    # 6) anomaly_invalid_composition + anomaly_imbalanced (NUEVO 2024):
    # movs sueltos con flag, sin transfer/fx group.
    for r in anomaly_invalid + anomaly_imbalanced:
        if not r["account_id"] or not r["movement_date"]:
            skipped += 1
            continue
        movements.append(build_movement(r, club_id, override_category_id=None))
    ...
    print(f"  Filas transfer_multi_source: {sum(len(v) for v in transfers_multi_grouped.values())} ({len(transfers_multi_grouped)} grupos)")
    ...
    print(f"  Filas anomaly_invalid_composition: {len(anomaly_invalid)}")
    print(f"  Filas anomaly_imbalanced: {len(anomaly_imbalanced)}")
