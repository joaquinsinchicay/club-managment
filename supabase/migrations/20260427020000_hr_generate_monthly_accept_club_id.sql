-- Fix: hr_generate_monthly_settlements devolvía `forbidden` porque
-- dependía de app.current_club_id seteado por `set_current_club`,
-- pero esa variable de sesión no persiste entre llamadas RPC
-- (PostgREST usa connection pooling, cada RPC es una transacción
-- independiente).
--
-- Fix: aceptar p_club_id como parámetro explícito, igual al pattern
-- ya usado por hr_create_contract_with_initial_revision
-- (migración 20260424010000_hr_fix_rpc_forbidden).
--
-- Los otros RPCs de liquidaciones (confirm, annul, pay) no necesitan
-- este fix porque derivan club_id del p_settlement_id que reciben.

drop function if exists public.hr_generate_monthly_settlements(int, int);
...
begin
  ...
  if v_current_club is not null and v_current_club <> p_club_id then
    return json_build_object('ok', false, 'code', 'forbidden');
  end if;
...
grant execute on function public.hr_generate_monthly_settlements(uuid, int, int) to authenticated;
