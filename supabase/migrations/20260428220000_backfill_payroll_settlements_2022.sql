-- Backfill de payroll_settlements 2022 desde los movimientos historicos
-- importados via CSV (scripts/import-2022/).
--
-- Mismo patron que el backfill 2021: 1 settlement por (contract, year, month)
-- con status='pagada', total_amount=SUM(amount), paid_movement_id=el mov mas
-- antiguo del par. Movs extras quedan con staff_contract_id pero
-- payroll_settlement_id NULL (siguen visibles en la ficha del colaborador via
-- el FK).
--
-- Idempotente: WHERE payroll_settlement_id IS NULL filtra solo pendientes.
...
mov_groups AS (
  SELECT
    ...
    (ARRAY_AGG(id ORDER BY (regexp_replace(external_id, '^IMP2022-', ''))::int))[1]
...
inserted AS (
  ...
  SELECT
    ...
    CASE WHEN mg.movs_count > 1
         THEN 'Carga historica CSV 2022 - agrupa ' || mg.movs_count || ' pagos parciales del mes.'
         ELSE 'Carga historica CSV 2022.'
    END,
    NOW(), NOW()
  FROM mov_groups mg
